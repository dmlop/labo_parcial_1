...
void AgregarPedido(ePedidos vector[],int tam,ePedidos aux,int* id, int auxId){
	...
	if(GetIntRango(&aux.kilos, "Ingrese la cantidad de kilos: ","ERROR DATO INVALIDO! VUELVA A INTENTARLO!\n", 100, 5000, 4)){
		aux.id=*id;
		aux.idC=auxId;
		if(CargarPedido(vector,tam,aux)==-1){
			printf("===NO=SE=PUDO=CARGAR=CLIENTE===\n");
		}
		else{
			printf("=====PEDIDO=CARGADO=ID=%d=====\n",*id);
			*id=*id+1;
		}
...
int ProcesarPedido(ePedidos* pedido){
	...
	do{
		switch(cPasos){
		case 1://PEDIR HDPE
			if(GetIntRango(&aux.HDPE, "Ingrese la cantidad de HDPE procesado: ","ERROR DATO INVALIDO! VUELVA A INTENTARLO!\n", 1, 5000, 4)){
				cPasos++;
			}
			else{
				cPasos=-1;
			...
			break;
		case 2://PEDIR LDPE
			if(GetIntRango(&aux.LDPE, "Ingrese la cantidad de LDPE procesado: ","ERROR DATO INVALIDO! VUELVA A INTENTARLO!\n", 1, 5000, 4)){
				cPasos++;
			}
			else{
				cPasos=-1;
			...
			break;
		case 3://PEDIR PP
			if(GetIntRango(&aux.PP, "Ingrese la cantidad de PP procesado: ","ERROR DATO INVALIDO! VUELVA A INTENTARLO!\n", 1, 5000, 4)){
				cPasos++;
			}
			else{
				cPasos=-1;
	...
	if(pedido->kilos>=totalPlasticos){
		...
		printf("ERROR! PLASTICO PROCESADO ES MAYOR QUE EL DISPONIBLE!\n");
	}
	return retorno;
}
...
void PedidosTesteo(ePedidos vector[],int *id,int actividad){
	...
	if(actividad==1){
		printf("=====PEDIDOS=DE=TESTEO=CARGADOS=====\n");
		vector[0].id=*id;
		vector[0].isEmpty=0;
		vector[0].idC=3;
		vector[0].estado=1;
